const d3 = Object.assign({}, require('d3-selection'), require('d3-scale'), require('d3-array'), require('d3-zoom'), require('d3-transition'), require('d3-timer'), require('d3-color'))
...
function ForceChart(svg, color) {
	...
	this.update = function (data) {
		...
		// Apply the general update pattern to the node labels.
		let nodeLabelSelection = self.textLabel.selectAll('.textLabel')
			  .data(labelData, d => d.key)
			  
		nodeLabelSelection.join(
		  enter => 
			  enter.append('text')
		...
		.call(all => all.transition(t)
		  ...
		  .style('fill', function(d) {return d3.color(color(d.community)).darker() })
		)
